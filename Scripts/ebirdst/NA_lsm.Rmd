...
NA_comp <- NA_comp[, -c(3,4,11,12,15:17, 40,41)]
```
...
write_sf(utrends_buff,"Predictor_buffer/utrends_buff.gpkg",
         layer = "ubuff")
...
utrends_buff <- st_read("Predictor_buffer/utrends_buff.gpkg", layer = "ubuff") #shapefile should be in WGS43


# Temporarily drop geometry for the join
upsabuff <- st_drop_geometry(utrends_buff)  

#remove unnecessary columns
upsabuff <- upsabuff[, -c(1:6)]

saveRDS(upsabuff, "Data/upsabuff.rds")
```


## Dont run, since im not using abundance I dont have to format by year. I will format change in predictor, leaving me with one estimate of each variable for each srd_id
Pivot upsabuff to long format for easier rbind
```{r}
ubuff_long <- upsabuff %>%
  pivot_longer(
...
```{r}
all_lsm <- NA_comp %>%    #phjv_lsm_comp has all estimates, since it wasnt subset by abd_ppy_no
...
#Final lsm df with all locations and all ebird data
NA_all_lsm <- ubuff_long %>%
  left_join(all_lsm, by = c("srd_id", "year"))
...
```{r, eval=FALSE}
#save as RDS
saveRDS(NA_all_lsm, "Data/NA_all_lsm.rds")

#save a CSV 
write.csv(NA_all_lsm, "Data/NA_all_lsm.csv", row.names = FALSE)
...
```{r}
#save as RDS
saveRDS(all_lsm, "Data/all_lsm.rds")

#save a CSV 
write.csv(all_lsm, "Data/all_lsm.csv", row.names = FALSE)
```
