...
# Select only the columns with "pland" 
pland_cols <- phjv_lsm_final %>%
  dplyr::select(contains("pland"))

# Calculate Pearson's correlation matrix
correlation_matrix <- cor(pland_cols, use = "pairwise.complete.obs", method = "pearson")
...
ggplot(data = correlation_melted, aes(x = Var1, y = Var2, fill = value)) +
  ...
  coord_fixed() +
  labs(title = "Correlation Matrix of Crop Proportions Metrics",
       x = "Variables",
       y = "Variables")
```
...
predictors <- c(
   "lpi_c11_Grassland", "ai_cNA_NA", "cohesion_cNA_NA", "shdi_cNA_NA", "lpi_c09_Forage", "pland_c02_Ag"
)
...
```{r}
ggplot(phjv_lsm_final, aes(y = abd_trend)) +
  geom_boxplot() +
  labs(title = "Boxplot of abd_trend", y = "Trend in Abundance", x = "") +
  theme_minimal()

ggplot(phjv_lsm_final, aes(y = abd)) +
  geom_boxplot() +
  labs(title = "Boxplot of abundance", y = "Abundance", x = "") +
  theme_minimal()
...
Trend
```{r}
shapiro.test(phjv_lsm_final$abd_trend)

hist(phjv_lsm_final$abd_trend, main = "Histogram of trend", xlab = "abd_trend")

qqnorm(phjv_lsm_final$abd_trend)
qqline(phjv_lsm_final$abd_trend, col = "red")


library(e1071)
skewness(phjv_lsm_final$abd_trend)
kurtosis(phjv_lsm_final$abd_trend)
...
abundance
```{r}
hist(phjv_lsm_final$abd, main = "Distribution of Abundance", xlab = "Abundance")

shapiro.test(phjv_lsm_final$abd)


qqnorm(phjv_lsm_final$abd)
qqline(phjv_lsm_final$abd, col = "red")
...
Abundance with year fixed effect
```{r}
library(lme4)
model1 <- lmer(abd ~ shdi_cNA_NA + ai_cNA_NA + lpi_c11_Grassland + lpi_c09_Forage + pland_c02_Ag + year + (1 | srd_id) , data = phjv_lsm_final)
...
Trend with year interaction
```{r}

year_int_model <- lmer(abd ~ shdi_cNA_NA * year + ai_cNA_NA * year + lpi_c11_Grassland * year + lpi_c09_Forage * year + pland_c02_Ag * year + (1 | srd_id), data = phjv_lsm_final)

qqnorm(residuals(year_int_model))
qqline(residuals(year_int_model), col = "red")
hist(residuals(year_int_model))

plot(fitted(year_int_model), residuals(year_int_model), 
     xlab = "Fitted Values", ylab = "Residuals", main = "Residuals vs Fitted")
abline(h = 0, col = "red")
...
model <- lm(abd_trend ~ shdi_cNA_NA + ai_cNA_NA + lpi_c11_Grassland + lpi_c09_Forage + pland_c02_Ag, 
              data = phjv_lsm_final)

summary(model)
```

Basic lm with year interaction
```{r}
# Model abundance as a function of SHDI and other predictors
fixedsrd_model <- lm(abd ~ shdi_cNA_NA * year + ai_cNA_NA * year + lpi_c11_Grassland * year + lpi_c09_Forage * year + pland_c02_Ag * year , 
              data = phjv_lsm_final)

summary(fixedsrd_model)
```



# GEOG 822 Exploratory Analysis
...
df_subset <- phjv_lsm_22 %>% 
  dplyr::select(abd, abd_trend, , lpi_c11_Grassland, shdi_cNA_NA, cohesion_cNA_NA, ai_cNA_NA, lpi_c09_Forage, pland_c09_Forage, pland_c01_Cereal )
